...
SELECT 
    s.flat_hash, s.link, s.title, s.price, s.is_apartament, s.is_studio, s.area, 
...
LEFT JOIN gold.history_flats h
    ON s.flat_hash = h.flat_hash AND h.is_active = TRUE
WHERE h.flat_hash IS NULL
   OR h.price != s.price;
